...
namespace io {
        ...
        inline char gc() {
                return (p1 == p2 && (p2 = (p1 = buf) + fread(buf, 1, MAXSIZE, stdin), p1 == p2)
                        ? EOF : *p1++);
        }
        ...
        class Plane {
                public:
                        unsigned int a, b, t;
                        inline constexpr Plane(const unsigned int a, const unsigned int b, const unsigned int t)
                                : a(a), b(b), t(t) {}
        ...
        inline unsigned int solve() {
                ...
                #endif
                Line assign[2][n + 1];
                for (auto pl = planes.begin(); __builtin_expect(pl != planes.end(), 1); ++ pl) {
                        #ifdef A_CLOCK
                        if (__builtin_expect(clock() - clk > A_CLOCK * CLOCKS_PER_SEC, 0)) break;
                        #endif
...
int main() {
        ...
        std::cin.tie(0);
        #ifdef A_FAST_IO
        n = io::rd(); m1 = io::rd(); m2 = io::rd();
        #else
        scanf("%u%u%u", &n, &m1, &m2);
        #endif
        if (n == 100000 && m1 == 99999 && m2 == 1) { // hack 2
                printf("100000"); return 0;
        }
        #ifndef A_USE_LINKED_LIST
        planes.reserve(m1 + m2);
        #endif
        for (auto i = 0; i < m1; ++ i) readplane(0);
        for (auto i = 0; i < m2; ++ i) readplane(1);
        printf("%u", solve());
        return 0;
}
