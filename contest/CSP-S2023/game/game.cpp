/**
 * 2023/11/11 CST 23:01
 * 你没看错，有傻子考场代码不小心写没个中括号然后零分了
 */
#include <bits/stdc++.h>
//#include <algorithm>
...
namespace aGame {
	...
	INLINE U sol() {
		U ans = 0;
		B *dp = (B*) malloc(sizeof (B) * (MAXN / 2) * MAXN); // �� len (MAXN / 2) �� start (MAXN)
		for (U len = 2, lenpos = 0; len <= n; len += 2, ++ lenpos) {
			B *dpl = &dp[lenpos * MAXN];
			#define DP(l, st) dp[((l) * MAXN) + (st)]
			if (NLKL(len == 2)) {
				for (U start = 0; start <= n - len; ++ start) {
					if (dpl[start] = s[start] == s[start + 1]) ++ ans;
				}
			} else {
				for (U start = 0; start <= n - len; ++ start) {
					#define SETYES { dpl[start] = 1; ++ ans; continue; }
					if (NLKL(len % 4 == 0)) {
						U hl = (lenpos - 1) / 2; // l / 2
						if (DP(hl, start) && DP(hl, start + (len / 2))) SETYES;
					}
					if (DP(lenpos - 1, start + 1) && s[start] == s[start + len - 1]) SETYES;
					if (s[start] == s[start + 1] && DP(lenpos - 1, start + 2)) SETYES;
					if (DP(lenpos - 1, start) && s[start + len - 1] == s[start + len - 2) SETYES;
					#undef SETYES
				}
			}
			#undef DP
